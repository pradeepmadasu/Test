...
//=================================================================================================================
	...
	$File = fopen('detail/detail-starmaker200.csv', 'w');
	fputcsv($File, array('Starmaker', 'Star/Downline', 'PV'));


	$Data = array();
// Made changes to modify PV to 200 from 300
	$Results = DatabaseQuery("SELECT MemberId, PV FROM Volume WHERE Year = $Year AND Month = $Month AND PV >= 200");
	$Count = 0;
...
// Made changes to modify PV to 200 from 300
		$Results2 = DatabaseQuery("SELECT Members.MemberId, PV FROM Members INNER JOIN Volume ON Members.MemberId = Volume.MemberId " .
			"WHERE Year = $Year AND Month = $Month AND PV >= 200 AND SponsorMemberId = {$Row['MemberId']} AND Type = 'D' ");
